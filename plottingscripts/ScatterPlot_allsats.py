...
def main():
  ...
  for k in range(len(season)):
    ...
    while day <= endday:
      ...
      plot1.add_title(label=f"HS Day {day} {model} {season[k]}")
      plot1.add_xlabel(xlabel=f"observation (satelites)")
      ...
      fig.plot_list = [plot1]
      fig.create_figure()
      fig.save_figure(f"scatter_HS_{model}_satelites_{season[k]}_day{day}.png")
      fig.close_figure()    
      ...
      plot1 = CreatePlot()
      plot1.plot_layers = [sctr1]
      plot1.add_title(label=f"WND Day {day} {model} {season[k]}")
      plot1.add_xlabel(xlabel=f"observation (satelites)")
      plot1.add_ylabel(ylabel=f"model ({model})")
      plot1.add_legend()
      plot1.set_xlim(0,35) 
