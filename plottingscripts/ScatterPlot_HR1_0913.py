...
def main():
  ...
  for k in range(len(season)):
    if season[k] == "winter":
       ...
       startdate = dt.datetime(2020,9,13)
       enddate = dt.datetime(2020,9,13)
       datestride = 1
       endday = 7
    ...
    for j in range(len(satelites)): 
      ...
      while day <= endday:
        ...
        fig.plot_list = [plot1]
        fig.create_figure()
        fig.save_figure(f"scatter_HS_{model}_{satelites[j]}_{season[k]}_day{day}_0913.png")
        fig.close_figure()    
        ...
        fig.plot_list = [plot1]
        fig.create_figure()
        fig.save_figure(f"scatter_WND_{model}_{satelites[j]}_{season[k]}_day{day}_0913.png")
        fig.close_figure()
