...
# Load configuration from JSON file, you can change the name
with open('configuration4.json', 'r') as f:
    config = json.load(f)
...
def create_combined_taylor_plot(data_dict, forecast_range, output_dir, variable):
    suffixes = ['gfsv16', 'multi1', 'hr1', 'hr2']
    sdev = []
    crmsd = []
    ccoef = []
    model_data = []
    ...
    for model_name in suffixes:
        if model_name in data_dict:
            ...
            # Calculate statistics for each model separately
    ...
    # Ensure all model data arrays are the same length
