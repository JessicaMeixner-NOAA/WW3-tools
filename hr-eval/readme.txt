...
5- buoyval-plst.py  , in this code, one can find the code to run the evaluation for the buoy validation outputs. like plotting the resluts and do the statistics. There is a switch in the configuration.json, 
if the user, make it true then it gives the all the buoys statistics separetly but if it is false you only get the all the buoys concatnation statistics in the format of the QQ-plot and the Talor diagram.
In the configuration.json, you can define the time windows that you are interested in calculating.
